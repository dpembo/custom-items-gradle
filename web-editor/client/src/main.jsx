...
import { AlertTriangle, CheckCircle2, CircleHelp, Copy, ImagePlus, LoaderCircle, Plus, Save, Trash2, Upload, X } from 'lucide-react';
import './styles.css';

const blankItem = (items = []) => ({ id: '', displayName: '', material: '', customModelData: Math.max(0, ...items.map(candidate => Number(candidate.customModelData) || 0)) + 1, texture: '', lore: [], enchantments: [], attributes: [] });
...
function App() {
  ...
  const startNewItem = () => { setSelectedId(null); setItem(blankItem(items)); setProblems([]); setToast(null); };
  const cloneItem = source => {
    const existingIds = new Set(items.map(candidate => candidate.id));
    let count = 2;
    let id = `${source.id}-${count}`;
    while (existingIds.has(id)) id = `${source.id}-${++count}`;
    setSelectedId(null);
    setItem({ ...structuredClone(source), id, displayName: `${source.displayName} ${count}`, customModelData: Math.max(0, ...items.map(candidate => Number(candidate.customModelData) || 0)) + 1 });
    setProblems([]); setToast(null);
  };
  const save = async () => {
    try {
  ...
  const remove = async () => { if (!selectedId || !confirm(`Delete ${selectedId}?`)) return; await request(`/api/items/${selectedId}`, { method: 'DELETE' }); setSelectedId(null); setItem(blankItem(items.filter(candidate => candidate.id !== selectedId))); await load(); };
  ...
  return <main>
    ...
    <aside><div className="sidebar-heading"><strong>Custom items</strong><button className="icon-button" title="New custom item" onClick={startNewItem}><Plus size={18} /></button></div><div className="item-list">{items.map(listItem => <button className={listItem.id === selectedId ? 'item-row active' : 'item-row'} onClick={() => select(listItem)} onContextMenu={event => { event.preventDefault(); cloneItem(listItem); }} title="Right-click to duplicate" key={listItem.id}><span className="item-glyph">{listItem.material.slice(0, 1)}</span><span><strong>{listItem.displayName}</strong><small>{listItem.id}</small></span><Copy className="clone-icon" size={14} /></button>)}</div></aside>
